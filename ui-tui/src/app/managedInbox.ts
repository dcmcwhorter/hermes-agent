...
const NON_PENDING_ACK_STATUSES = new Set(['started', 'completed', 'blocked', 'error'])

const latestAckStatusByInboxId = (dir: string) => {
  const latest = new Map<string, string>()

  for (const row of jsonlRows(join(dir, 'acks.jsonl'))) {
    if (typeof row.inbox_id === 'string' && typeof row.status === 'string') {
      latest.set(row.inbox_id, row.status)
    }
  }
...
const pendingManagedItems = (
  ...
  const latestAckStatus = latestAckStatusByInboxId(dir)
  ...
  return jsonlRows(join(dir, 'inbox.jsonl'))
    ...
    .filter(item => !NON_PENDING_ACK_STATUSES.has(latestAckStatus.get(item.id || '') || ''))
...
3. Call mcp_gateway_list_tasks({"status":"all"}). Do not skip this call because the briefing says there are no active tasks.
