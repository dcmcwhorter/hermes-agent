...
describe('managed inbox helpers', () => {
  ...
  it('keeps submitted non-lifecycle items pending until a stronger status exists', () => {
    const dir = tempDir()
    appendJsonl(join(dir, 'inbox.jsonl'), [
    ...
    expect(pendingManagedInboxItems(dir).map(item => item.id)).toEqual(['msg-1', 'msg-2'])
  })
  ...
  it('keeps submitted-only inbox items pending until a stronger lifecycle status exists', () => {
    const dir = tempDir()
    appendJsonl(join(dir, 'inbox.jsonl'), [
      { id: 'msg-1', kind: 'message_wake', text: 'call check_messages' },
      { id: 'msg-2', kind: 'message_wake', text: 'completed already' }
    ])
    appendJsonl(join(dir, 'acks.jsonl'), [
      { inbox_id: 'msg-1', trigger: 'message_wake', status: 'submitted' },
      { inbox_id: 'msg-2', trigger: 'message_wake', status: 'completed' }
    ])

    expect(pendingManagedInboxItems(dir).map(item => item.id)).toEqual(['msg-1'])
  })
  ...
  it('builds lifecycle prompt with exact gateway calls', () => {
    ...
    expect(prompt).toContain('mcp_gateway_list_tasks({"status":"all"})')
  })
})
