    ...
    try:
        ...
        if not safe_urls:
            ...
            if provider is None or not provider.supports_extract():
                # Search-only configured backends (ddgs/brave/searxng) fall
                # back to direct HTTP extraction instead of requiring a paid
                # provider. Unknown or unavailable providers first try any
                # active extraction plugin, then use the same bounded fallback.
                if provider is not None and not provider.supports_extract():
                    ...
                    if provider is None:
                        logger.info(
                            "No configured extract backend available; using direct HTTP fallback"
                        )
                        results = await _direct_http_extract_urls(safe_urls)
                    else:
                        logger.info(
